...
export default function ReportsPage() {
  ...
  const reportRow = (
    ...
    <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 last:border-0 sm:px-5">
      ...
      >
        <span translate="no" aria-hidden="true" className="material-symbols-outlined notranslate text-[16px]">download</span>
        {busy === key ? "匯出中…" : "匯出 CSV"}
      </button>
    </div>
  );
          ...
          {kpiCards.map((c) => (
            <div key={c.label} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <span translate="no" aria-hidden="true" className={`material-symbols-outlined notranslate text-[22px] ${c.accent}`}>{c.icon}</span>
              </div>
