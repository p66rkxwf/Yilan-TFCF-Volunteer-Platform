...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    question: "忘記密碼怎麼辦？",
    answer:
      "本平台以「帳號」登入，暫不提供自動寄送重設密碼信的功能。請點選登入頁的「忘記密碼」，再前往支援頁面留下您的帳號與問題，由平台管理團隊核對身分後協助重設。重設後系統會產生一組臨時密碼，由承辦人員轉告您；以該臨時密碼登入後，系統會要求您立即設定新密碼。",
  },
  {
    category: "活動報名",
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex h-11 items-center rounded-lg border border-slate-200 bg-white px-3">
          <span translate="no" aria-hidden="true" className="material-symbols-outlined notranslate text-[20px] text-slate-400">search</span>
          <input
            type="text"
            value={searchQuery}
      ...
      <div>
        ...
        {filteredItems.length > 0 ? (
            ...
            {filteredItems.map((item, index) => (
              <details key={item.question} open={index === 0} className="group py-1">
                <summary className="flex cursor-pointer items-center justify-between gap-4 py-3 text-left">
                  ...
                  <span translate="no" aria-hidden="true" className="material-symbols-outlined notranslate shrink-0 text-[20px] text-slate-400 transition-transform group-open:rotate-180">
                    expand_more
                  </span>
                </summary>
          ...
          <div className="py-12 text-center">
            <span translate="no" aria-hidden="true" className="material-symbols-outlined notranslate text-4xl text-slate-300">search_off</span>
      ...
      <div className="border-t border-slate-200 pt-6">
        ...
        <div className="mt-2 flex flex-col">
          {QUICK_LINKS.map((link) => (
            ...
            >
              <span translate="no" aria-hidden="true" className="material-symbols-outlined notranslate text-[20px] text-primary">{link.icon}</span>
              <span>
